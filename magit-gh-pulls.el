...
(defun magit-gh-pulls-guess-repo-from-origin ()
  (let ((url (magit-get "remote" "origin" "url")))
    (when (and url (s-matches? "^git@github.com" url))
      (let ((split (s-match "^git@github.com:\\(.+\\)/\\(.+\\).git$" url)))
        (cons (cadr split) (caddr split))))))
                     ...
                     (header (concat (format "[%s@%s] " id
                                     ...
                                     (propertize (format "%s\n" (oref req :title))
                                                 'face (cond (have-commits 'default)
                                                             (invalid 'error)
                                                             (t 'italic))))))
                (magit-with-section id (cond ((not (equal branch base-ref)) 'not-on-current-branch)
                                             (have-commits 'pull)
...
(defun magit-gh-pulls-merge-pull-request ()
  ...
  (magit-section-action (item info "ghpr")
    ...
    ((not-on-current-branch)
     (error "Checkout to the branch this pull request was made for"))
    ((invalid-pull)
     (error "This pull request refers to invalid reference"))))
...
(define-minor-mode magit-gh-pulls-mode "Pull requests support for Magit"
  ...
  (if magit-gh-pulls-mode
      (progn
        (add-hook 'magit-before-insert-unpushed-commits-hook 'magit-insert-gh-pulls nil t))
    (progn
      (remove-hook 'magit-before-insert-unpushed-commits-hook 'magit-insert-gh-pulls t)))
  (when (called-interactively-p 'any)
    (magit-refresh)))

(remove-hook 'magit-after-insert-pending-commits-hook 'magit-insert-gh-pulls t)
